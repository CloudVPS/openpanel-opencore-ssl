...
typedef enum {cs_disconnected, cs_accept, cs_getreq, cs_connected, cs_closing} ConnStatus;
typedef struct {
	...
	struct sockaddr_in peer_sa;			// The peer's sockaddr.
	int peer_sa_len;					// ^^^^^^^^^^^^^^^^^^^^^^^'s len
...
int conn_accept(void) {
	...
	conn[i].peer_sa_len = sizeof (struct sockaddr_in);
	getpeername (s, (struct sockaddr *) &conn[i].peer_sa, &conn[i].peer_sa_len);
...
void sighandler(int signum) {
	switch (signum) {
		...
		case SIGTERM:
			...
			if (!chroot_dir) unlink("/var/run/opencore-ssl.pid");
			exit(0);
		default:;
	}
}
...
int main(int argc, char **argv) {
	...
	client_port = 4088;
	cert_file = key_file = "/etc/openpanel/openpanel.pem";
	set_uid = "nobody";
	...
	if (!debug_flag && !fg_flag && (pid=fork())) {
		pidfile=fopen("/var/run/opencore-ssl.pid", "w");
		if (pidfile) {
			fprintf(pidfile, "%d\n", pid);
			fclose(pidfile);
		}
...
//								cn->c_end_req=1;
					...
					if (cn->stat == cs_getreq)
					{
						// X-Originating-Address: 255.255.255.255
						char *eol;
						eol = memchr (cn->csbuf, '\n', cn->csbuf_e - cn->csbuf);
						if (eol == NULL) break;
						if (((cn->csbuf_e+64)-cn->csbuf) > cs_buflen)
						{
							cn->stat = cs_closing;
							event=1;
						...
						else
						{
							eol++;
							size_t strsz;
							unsigned int i = ntohl (cn->peer_sa.sin_addr.s_addr);
							sprintf (pbuf, "X-Originating-Address: %i.%i.%i.%i\r\n",
									 (i & 0xff000000) >> 24, (i & 0xff0000) >> 16,
									 (i & 0xff00) >> 8, i & 0xff);
									 
							strsz = strlen (pbuf);
							
							memmove (eol+strsz, eol, cn->csbuf_e - eol);
							memcpy (eol, pbuf, strsz);
							cn->csbuf_e += strsz;
